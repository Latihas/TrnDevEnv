...
public class ScriptUwU : IScriptBase {
	...
	private readonly Jobs[] JobOrder = [Jobs.MT, Jobs.ST, Jobs.H1, Jobs.H2, Jobs.D1, Jobs.D2, Jobs.D3, Jobs.D4];
	...
	private CancellationTokenSource? ctssimple, ctsp1, ctsp2, ctsp3;
	private string? hs, ttsafe, hsadd;
	private int p43dhCount;
	private List<DHP43> p43dhDist = [];
	private State state;
	private bool Ubroadcast, Uthreebucket, Uauto, Umarklocal, Umark, Utarget;
	...
	public override List<(Regex, Action<GroupCollection>)> CustomList => [
		...
		new(new Regex(@"^.{14} AddCombatant 03:(?<zid>.{8}):.+:00::2091:8726:"), g => {
			lock (ciyu) ciyu.zid = g["zid"].Value;
		}),
		new(new Regex(@"^.{14} (?:ActionEffect 15|AOEActionEffect 16):.{8}:(?<sname>[^:]+):(?<jid>[^:]+):(?<jname>[^:]+):.+:71878:0:10000"), g => {
			...
			lock (ciyu) ciyu.dmg.Append(g["sname"].Value).Append('[').Append(g["jname"].Value).Append(']');
		}),
	...
	private void markerBelowHPtarget(string marker, double percent, string hexid) {
		foreach (var en in BridgeFFXIV.GetAllEntities().Where(x => x.GetValue("Marker").ToString() == marker)) {
			var isLowHp = 1.0 * int.Parse(en.GetValue("currenthp").ToString()) / int.Parse(en.GetValue("maxhp").ToString()) < percent;
			var hasLowStack = int.Parse(en.GetValue("StatusStack(609)").ToString()) <= 1;
	...
	private void Newp2FindTargetzhuzi(int a1, int a2, int a3, int a4) {
		int targetIndex;
		switch (MyJob) {
			case Jobs.D1:
				targetIndex = a1;
		...
		// if (targetIndex == -1 || targetIndex >= Zhuzis.Count) return;
		var targetZid = Zhuzis[targetIndex].zid;
		if (Utarget) Target(targetZid);
		if (Utarget) PostTip("已切换目标");
		if (Utarget) markerBelowHPtarget(targetZid, 0.4, FindTargetHexIdByBnpcId("8730"));
	}
	...
	private void Mark(object hexId, string marktype) {
		if (Umarklocal && Umark) RealPlugin.Instance.InvokeNamedCallback("mark", $"{{\"ActorID\":0x{hexId},\"MarkType\":\"{marktype}\",\"LocalOnly\":\"True\"}}");
		if (!Umarklocal && Umark) RealPlugin.Instance.InvokeNamedCallback("mark", $"{{\"ActorID\":0x{hexId},\"MarkType\":\"{marktype}\"}}");
	}

	private void Target(object hexId) {
		if (Utarget) RealPlugin.Instance.InvokeNamedCallback("Target", $"{{\"ActorID\":0x{hexId}}}");
	}

	private string FindTargetHexIdByBnpcId(string targetBnpcId) {
	...
	private void p43qdptrace(string xy) {
		var pos = GetXY(xy);
		Place($"2:{pos.X},{pos.Y}");
		if (pos is { X: > 100, Y: > 100 }) Broadcast("警告，潜地炮位置出现在右下");
	}

	private void xfzd(string time) {
	...
	private void p3nl(string id) {
		Broadcast("优先攻击石牢");
		Mark(id, "attack1");
	}

	private void rf(string name) {
		var HDead = false;
		foreach (var p in Players) {
			if (BridgeFFXIV.GetNamedPartyMember(p.name).GetValue("currenthp").ToString() == "0" && p.job is Jobs.H1 or Jobs.H2) {
				Log("Dead: " + p.name);
	...
	private void p41taitan(string x) {
		var dir = x switch {
			"113.70" => "右右右",
			"86.30" => "左左左"
		};
...
// 示例 DecodePos 方法（根据实际数据调整）
	...
	private void NEWp3ygjd(string xy) {
		// 输入验证
		var ss = xy.Split(',');
	...
	private string GetPScale(string name) => GetScalarVariable(true, "ptuw_" + name);

	private string GetScale(string name) => GetScalarVariable(false, "tuw_" + name);

	private void p3fly(string id) {
		var pos = GetXY(id);
		ttsafe = "";
	...
	private void PostTip(string text, float x = 0, float y = 0, string id = "main", bool tts = true) {
		//TODO
	...
	private void FsPlace(string pl1, string pl2) {
		Place(pl1 switch {
	...
	public void FsPlaceByRule(string pos1, string pos2) {
		// 3点位优先顺序
	...
	private string GetDir(Vector2 v) => GetDir(v.X, v.Y);

	private string GetDir(string x, string y) => GetDir(float.Parse(x), float.Parse(y));
	...
	private List<Vector2> GetXYFromBnpcid(string arg, int reqHP = -1) {
	...
	private Vector2 GetXY(string id_xy) {
		if (id_xy.Contains(",")) {
			var ss = id_xy.Split(',');
	...
	private void GetPartyOrderInit(string args) {
		var ss = args.Split(',');
		var find = false;
		for (var i = 0; i < 8; i++) {
			var job = JobOrder[i];
