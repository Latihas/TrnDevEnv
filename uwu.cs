...
// using static Triggernometry.Core.ActionOld.ActionTypeEnum;
// using static Triggernometry.FFXIV.Job.RoleType;
// using static Triggernometry.Core.RealPlugin;
// using Action = Triggernometry.Core.ActionOld;
//
// ;
// Instance.UnregisterNamedCallback("LatihasTUW");
// Instance.RegisterNamedCallback("LatihasTUW", new Action<object, string>(LatihasTUW.Callback), null);
//  
//
// public static class LatihasTUW {
//  static Context fakectx = new Context(null);
...
//     }
//    }
//    Instance.InvokeNamedCallback("place", sb.Append("}").ToString());
//   }
//   catch (Exception ex) {
...
//   if (Umarklocal&&Umark) Instance.InvokeNamedCallback("mark", $"{{\"ActorID\":0x{hexId},\"MarkType\":\"{marktype}\",\"LocalOnly\":\"True\"}}");
//   if (!Umarklocal&&Umark) Instance.InvokeNamedCallback("mark", $"{{\"ActorID\":0x{hexId},\"MarkType\":\"{marktype}\"}}");
//  }
//  private static void Target(object hexId) {
//   if(Utarget)Instance.InvokeNamedCallback("Target", $"{{\"ActorID\":0x{hexId}}}");
//  }
//
...
//  
//  // 绘制原始点
//  Instance.InvokeNamedCallback("PictoACT", $"Omen: er_general_1f\nt: 25\nPos: {startPoint.X}, {startPoint.Y}\nScale: 1,1\nColor: 1, 1, 1, 0.9");
//  if (Uauto)
//  {
//   Instance.InvokeNamedCallback("command", $"/e 原始点:({startPoint.X:F1}, {startPoint.Y:F1})");
//  }
//  
//  // 顺时针旋转三次：30度, 30度, 30度
...
//   if (Uauto)
//   {
//    Instance.InvokeNamedCallback("command", $"/e 顺时针第{i+1}次旋转(累计{currentAngle}度) - 点:({rotatedPoint.X:F1}, {rotatedPoint.Y:F1})");
//   }
//  }
//  
//  // 逆时针旋转三次：30度, 30度, 30度
...
//   if (Uauto)
//   {
//    Instance.InvokeNamedCallback("command", $"/e 逆时针第{i+1}次旋转(累计{Math.Abs(currentAngle)}度) - 点:({rotatedPoint.X:F1}, {rotatedPoint.Y:F1})");
//   }
//  }
// }
//
...
//    if (Uauto)
//    {
//     Instance.InvokeNamedCallback("command", $"/e 顺时针第{i+1}次旋转(累计{currentAngle}度) - 点:({rotatedPoint1.X:F1}, {rotatedPoint1.Y:F1})");
//    }
//   }
//   
...
//    if (Uauto)
//    {
//     Instance.InvokeNamedCallback("command", $"/e 逆时针第{i+1}次旋转(累计{currentAngle}度) - 点:({rotatedPoint1.X:F1}, {rotatedPoint1.Y:F1})");
//    }
//   }
//  }
//
...
//   Instance.InvokeNamedCallback("command",$"/e 柱子标记完成");
//   
//   
//  }
//  
...
//   
//   if (Uauto)Instance.InvokeNamedCallback("Command" ,$"/e 地火D1坐标：({D1resultPoint.X}, {D1resultPoint.Y})");
//   if (Uauto)Instance.InvokeNamedCallback("Command" ,$"/e 地火D2坐标：({D2resultPoint.X}, {D2resultPoint.Y})");
//   if(Uauto)Instance.InvokeNamedCallback("Command" ,$"/e 地火第四次移动坐标：({intersection.X}, {intersection.Y})");
//  }
//
...
//     if(Uauto)Instance.InvokeNamedCallback("Command" ,$"/e 地火D4第三次移动坐标：({finalPoint1.X}, {finalPoint1.Y})");
//
//     // 第二次旋转：45度
...
//     if(Uauto)Instance.InvokeNamedCallback("Command" ,$"/e 地火D4第二次移动坐标：({finalPoint2.X}, {finalPoint2.Y})");
//
//     // 第三次旋转：33度
...
//     if(Uauto)Instance.InvokeNamedCallback("Command" ,$"/e 地火D4第一次坐标：({finalPoint3.X}, {finalPoint3.Y})");
//
//
//     // 顺时针旋转
//     Console.WriteLine("\n顺时针旋转：");
...
//     Instance.InvokeNamedCallback("PictoACT", $"Omen: er_general_1f\nt: 20\nPos: {finalPoint1.X}, {finalPoint1.Y}\nScale: 1,1\nColor: 0, 0.3, 0.5");
//     if(Uauto)Instance.InvokeNamedCallback("Command" ,$"/e 地火D3第三次移动坐标：({finalPoint1.X}, {finalPoint1.Y})");
//
//     // 第二次旋转：-45度
//     translatedPoint = finalPoint1 - center;
...
//     if(Uauto)Instance.InvokeNamedCallback("Command" ,$"/e 地火D3第二次移动坐标：({finalPoint2.X}, {finalPoint2.Y})");
//     Instance.InvokeNamedCallback("PictoACT", $"Omen: er_general_1f\nt: 20\nPos: {finalPoint2.X}, {finalPoint2.Y}\nScale: 1,1\nColor: 0, 0.2, 0.7");
//
//     // 第三次旋转：-33度
//     translatedPoint = finalPoint2 - center;
...
//  
//     Instance.InvokeNamedCallback("PictoACT", $"Omen: er_general_1f\nt: 20\nPos: {finalPoint3.X}, {finalPoint3.Y}\nScale: 1,1\nColor: 0, 0.2, 0.9");
//     if(Uauto)Instance.InvokeNamedCallback("Command" ,$"/e 地火D3第一次移动坐标：({finalPoint3.X}, {finalPoint3.Y})");
// }
//
//
//
...
//         Instance.InvokeNamedCallback("PictoACT", $"Omen: er_general_1f\nt: 2\nPos: {posValue}\nScale: 1,1\nColor: 0.1, 1, 0.1, 0.9");
//         if (Uauto) Instance.InvokeNamedCallback("command", $"/e 3桶移动坐标:{posValue}");
//     }
//
//     // ttsafe 分支逻辑
//     int? index = null;
...
//             Instance.InvokeNamedCallback("PictoACT", "Omen: er_general_1f \nt: 2\n Pos: 100,106\n Scale: 1,1\n Color: 0.1, 1, 0.1, 0.9");
//             index = 0;
//             if(Uauto)Instance.InvokeNamedCallback("command","/e 1桶移动坐标:100,106");
//             break;
//         case "B点右东安全":
//             Place("1:94,100;2:100,100");//D1
//             Instance.InvokeNamedCallback("PictoACT", "Omen: er_general_1f \nt: 2 \nPos: 94,100 \nScale: 1,1 \nColor: 0.1, 1, 0.1, 0.9");
//             index = 1;
//             if(Uauto)Instance.InvokeNamedCallback("command","/e 1桶移动坐标:94,100");
//             break;
//         case "C点下南安全":
//             Place("1:100,94;2:100,100");//A1
//             Instance.InvokeNamedCallback("PictoACT", "Omen: er_general_1f \nt: 2 \nPos: 100,94 \nScale: 1,1 \nColor: 0.1, 1, 0.1, 0.9");
//             index = 2;
//             if(Uauto)Instance.InvokeNamedCallback("command","/e 1桶移动坐标:100,94");
//             break;
//         case "D点左西安全":
//             Place("1:106,100;2:100,100");
//             Instance.InvokeNamedCallback("PictoACT", "Omen: er_general_1f \nt: 2 \nPos:106,100 \nScale: 1,1 \nColor: 0.1, 1, 0.1, 0.9");
//             index = 3;
//             if(Uauto)Instance.InvokeNamedCallback("command","/e 1桶移动坐标:106,100");
//             
//             break;
//     }
//
...
//      Instance.InvokeNamedCallback("PictoACT", $"Omen: er_general_1f \nt: 5 \nPos: {posValue} \nScale: 1,1\nColor: 0.1, 1, 0.1");
//      if(Uauto)Instance.InvokeNamedCallback("command", $"/e 大怒震移动坐标:{posValue} ");
//     }
//
//     PostTip(ygjddir);
// }
...
//   Instance.InvokeNamedCallback("command", Ubroadcast ? $"/p {s} " : $"/e {s}");
//  }
//
...
//   Instance.QueueAction(fakectx, _tri, null,
//    new Action {
...
//       else Threebuckets.Add(v);
//       if (Uthreebucket) {
//        Instance.InvokeNamedCallback("command", $"/mk attack1 <{Threebuckets[0].partyorder}>");
//        Instance.InvokeNamedCallback("command", $"/mk attack2 <{Threebuckets[1].partyorder}>");
//        Instance.InvokeNamedCallback("command", $"/mk attack3 <{Threebuckets[2].partyorder}>");
//        
...
//         Instance.InvokeNamedCallback("command", "/mk attack1 <attack1>");
//         Instance.InvokeNamedCallback("command", "/mk attack2 <attack2>");
//         Instance.InvokeNamedCallback("command", "/mk attack3 <attack3>");
//        });
//       }
//       else {
//        Instance.InvokeNamedCallback("command", $"/e attack1 <{Threebuckets[0].partyorder}>");
//        Instance.InvokeNamedCallback("command", $"/e attack2 <{Threebuckets[1].partyorder}>");
//        Instance.InvokeNamedCallback("command", $"/e attack3 <{Threebuckets[2].partyorder}>");
//        
//        // 添加个人提示，让玩家知道自己点的是哪个桶
//        if (Threebuckets[0].name == MyName) PostTip("一桶点你");
//        else if (Threebuckets[1].name == MyName) PostTip("二桶点你");
...
//   Instance.InvokeNamedCallback("command", $"/e {message}");
//  }
//
